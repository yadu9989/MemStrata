"""V5.2-C Phase C.8 — coverage for memory_layer_pro.ollama_health.
...
class TestOllamaSyncStatusClassification:
    def test_ollama_unreachable_returns_unreachable(self):
        ...
        with patch(
            "memory_layer_pro.ollama_health.urllib.request.urlopen",
...
# ── Model pull NDJSON (§9.1) ──────────────────────────────────────────────
#
# The pull helper lives in wizard.py (kept stdlib-only there). We test
# it here because it's the natural home for "ollama_health & friends"
# coverage. The wizard module already imports clean.

class TestModelPullNDJSON:
    def test_model_pull_progress_parses_ndjson(self):
        """NDJSON progress events parsed correctly; renderer fires per event."""
        from memory_layer_pro import wizard

        events = [
            {"status": "pulling manifest"},
            {
                "status": "downloading", "digest": "sha256:abc",
                "total": 4_000_000_000, "completed": 1_000_000_000,
            },
            {
                "status": "downloading", "digest": "sha256:abc",
                "total": 4_000_000_000, "completed": 2_500_000_000,
            },
        ...
        def _capture(completed: int, total: int) -> None:
            progress_calls.append((completed, total))

        # The pull helper streams the response object line-by-line.
        # _FakeStreamResponse implements __iter__ to mimic urllib.
        class _FakeStream:
        ...
        with patch(
            "memory_layer_pro.wizard.urllib.request.urlopen",
            return_value=_FakeStream(ndjson),
        ), patch.object(wizard, "_render_progress", side_effect=_capture):
            wizard._pull_model_with_progress("qwen2.5-coder:7b")

        # Two "downloading" events → at least two render_progress calls
        # (plus one final "complete" render at success time).
        assert len(progress_calls) >= 2
        # The final render bumps to the total — last call is (total, total).
        assert progress_calls[-1] == (4_000_000_000, 4_000_000_000)

    def test_model_pull_handles_error_event(self):
        """Ollama error event → RuntimeError surfaced to caller."""
        from memory_layer_pro import wizard
        ...
        with patch(
            ...
            with pytest.raises(RuntimeError, match="model not found"):
                wizard._pull_model_with_progress("nonexistent:model")

    def test_model_pull_ignores_blank_lines(self):
        """Empty NDJSON lines must not crash the parser."""
        from memory_layer_pro import wizard
        ...
        with patch(
            "memory_layer_pro.wizard.urllib.request.urlopen",
            return_value=_FakeStream(ndjson),
        ), patch.object(wizard, "_render_progress"):
            wizard._pull_model_with_progress("qwen2.5-coder:3b")
